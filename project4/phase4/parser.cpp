...
static Type multiplicativeExpression(bool &lvalue)
{
	...
	while (1) {
		if (lookahead == '*') {
			...
			cout << "check *" << endl;
			right = checkMultiplicativeExpression(left, right, "*");
			lvalue = false;
...
static Type relationalExpression(bool &lvalue)
{
	...
	while (1) {
		if (lookahead == '<') {
			...
			cout << "check <" << endl;
			right = checkEqualityandRelational(left, right, "<");
			lvalue = false;
